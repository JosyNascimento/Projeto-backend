...
const createTicket = async (req, res) => {
  try {
    ...
    const ticket = await ticketService.createTicketFromCart(cid, userId);

    return res
      .status(201)
      .json({ message: "Compra realizada com sucesso", ticket });
  } catch (error) {
...
const getTicketsByUserId = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao buscar tickets do usuário" });
  }
};
...
const updateTicket = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao atualizar ticket do usuário" });
  }
};
